...
- 舞台顶部控制条使用单一“录制&分享”入口，独立面板分为“录制”“导入”和“曲库”：新录制默认 30 秒，可切换 60 秒；第一次发声建立录制零点，最后 5 秒状态条变红并逐秒倒计时。曲目名称使用 UTF-8，最多 8 字节；命名分享使用 `DGL1` 外层封装名称与原始 `DGT2` 数据，旧 `DGT1 / DGT2` 继续兼容。“生成分享链接”把命名分享码放进飞书妙搭 URL 的 `dj` 查询参数；接收页解析并校验后，在首次点击解锁音频后自动进入 DJ 模式播放。导入会自动恢复名称并存入 `localStorage` 曲库，曲库最多 50 首，可查看、改名、播放、导出和删除。导入页可开启循环播放。按音频时钟保存双押、第三行长按、同 Deck 滑音与松开时间。`DGT2` 使用 4–7 字节头部、紧凑二进制、可选 LZSS、Base64url 与 CRC32，立体声只保存开关；最长 90 秒的旧录制仍可导入。回放通过 Web Audio 前瞻调度复现演奏，并用虚拟触点同步重现按下、长按、滑音、松手与跟手图片；结束后恢复原设置。
- 音游设置项显示为“音游模式（不好玩）”。
