...
- 主界面与设置菜单均提供 3D 音效开关，两处状态与云端设置同步：开启时优先使用 Web Audio HRTF，将 Deck 放进带左右、前后与远近的三维声场；关闭时旁路 HRTF，保持原始直出。舞台滑杆可实时移动音场；iPad 可授权重力感应控制，3° 静区后在 15° 达到满幅，权限或传感器不可用时回到手动模式，背景音乐保持居中。
- PC 可用 `Z`、`/`、`B` 将音场切到左侧、右侧和中央；快捷键会从重力控制切回手动控制。
...
- 舞台顶部控制条使用单一“录制&分享”入口，独立面板分为“录制”和“导入”：新录制默认 30 秒，可切换 60 秒；第一次发声建立录制零点，最后 5 秒状态条变红并逐秒倒计时，完成后直接展示分享码与复制按钮。导入页可开启循环播放。按音频时钟保存双押、第三行长按、同 Deck 滑音与松开时间。`DGT2` 分享码使用 4–7 字节头部、紧凑二进制、可选 LZSS、Base64url 与 CRC32，3D 音效只保存开关，播放沿用当前音场位置；旧 `DGT1` 和最长 90 秒的旧录制仍可导入。回放通过 Web Audio 前瞻调度复现演奏，结束后恢复原设置。
...
- 每个短音、长音纹理和释放尾音共用同一 Deck 空间输出；开启 3D 时走 HRTF，浏览器缺少 HRTF 时回退到立体声声像，关闭后走原始旁路。
