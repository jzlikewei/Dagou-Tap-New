...
- DJ 设置新增音游入口：每局按 128 BPM 随机生成 8–24 组四小节乐句，即 1–3 分钟。每组由主题重复、变奏、跨 Deck 对答和收束组成，同一乐句只在相邻音高列移动，并按音色穿插“大狗叫”“大狗大狗叫叫叫”“叮咚叮咚鸡”等固定音节模板。
- 目标使用完整 `4 × 3` Deck 网格，2 Deck 与 3 Deck 分别提供 24 和 36 个区域；谱面包含跨 Deck 双押和第三行连音链。连音链完整判定起按、相邻目标格滑动与末端松手，期间只在其他 Deck 安排单点，整张谱面最多同时使用两根手指。
- 音游入口提供手动与 `AUTO 自动演奏` 两种启动方式。自动演奏通过正式输入链路触发声音、区域闪光、角色动画与跟手特效，单点保持 80 毫秒，双押同帧触发，连音链会真实拖到相邻格并在末端自动释放；HUD 与结算标题会标明自动状态。
