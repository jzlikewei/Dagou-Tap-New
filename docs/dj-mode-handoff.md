...
- DJ 模式与钢琴模式互斥；开启其中一个会关闭另一个。旧云端状态同时开启时，显式保存的 DJ 选择优先；只保存过钢琴选择时延续钢琴模式。
...
- DJ 设置新增音游入口：每局按 128 BPM 随机生成 8–24 组四小节乐句，即 1–3 分钟。四组依次分为主题、问答、提升、收束，组内交替重复、接龙、留白和双押落点。同一句音节模板有多种节奏型，音高只在相邻列移动。
- 目标使用完整 `4 × 3` Deck 网格，2 Deck 与 3 Deck 分别提供 24 和 36 个区域；谱面包含跨 Deck 双押、第三行长音和连音链。连音链完整判定起按、相邻目标格滑动与末端松手，期间只在其他 Deck 安排单点，整张谱面最多同时使用两根手指。
- 音游入口提供手动与 `AUTO 自动演奏` 两种启动方式。自动演奏通过正式输入链路触发声音和完整判定，单点保持 80 毫秒，双押同帧触发，连音链会真实拖到相邻格并在末端自动释放；HUD 与结算标题会标明自动状态。游玩期间每次从纸屑、折线、几何雨和星星中随机选择，特效以目标格为中心缩小到 34%，420 毫秒后开始退场。点击格整块按音色着色，并显示大狗、哈基米或叮咚鸡的张嘴图片；长尾迹收起，保留小范围扩散环或 Emoji 弹出。
