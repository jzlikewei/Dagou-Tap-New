...
- DJ 设置可在普通几何尾迹与大狗、猫、鸡图片尾迹之间切换，角色图片跟随所在 Deck 当前绑定的音色变化，两种样式共用触点双圆环；松手时图片轻微放大，并在约 200 毫秒内沿滑动方向飞出、淡出，原地松手默认向上飞。
...
- 主界面与设置菜单均提供双声道立体声开关，两处状态与云端设置同步。一栏保持居中，两栏分布在左、右，三栏分布在左、中、右；关闭后所有音效回到中央。实现只使用 `StereoPannerNode`，背景音乐保持居中。
- DJ、音游、钢琴模式互斥；开启其中一个会关闭其余模式。旧云端状态冲突时，显式保存的音游选择优先，其次为 DJ 与钢琴。
...
- 手机与平板设置提供“横屏演奏”，选择保存在当前设备。开启时先请求页面全屏，再通过 Screen Orientation API 请求锁定横屏；浏览器未授权时显示手动旋转提示，并保留“尝试自动横屏”与关闭入口。关闭时只退出页面自行进入的全屏。
- 音游改为独立模式开关：每局按 128 BPM 随机生成 8 组四小节乐句，共 32 小节，固定 1 分钟。四组依次分为主题、问答、提升、收束，组内交替重复、接龙、留白和双押落点。同一句音节模板有多种节奏型，音高只在相邻列移动。每四小节稳定安排 2–3 个持续句尾，由“叫、米、鸡”承载长音，其他栏穿插短句。
- 音游支持 1–3 栏，每栏使用完整 `4 × 3` 网格，分别提供 12、24、36 个区域；2–3 栏谱面包含跨栏双押，所有栏数都支持第三行长音和连音链。连音链完整判定起按、相邻目标格滑动与末端松手，期间只在其他栏安排单点，整张谱面最多同时使用两根手指。
- 音游入口提供手动与 `AUTO 自动演奏` 两种启动方式。自动演奏通过正式输入链路触发声音和完整判定，独立短音保持 120 毫秒，句内连奏保持 200 毫秒，持续句尾按谱面时值释放；双押同帧触发，连音链会真实拖到相邻格并在末端自动释放。HUD 与结算标题会标明自动状态。游玩期间每次从纸屑、折线、几何雨和星星中随机选择，特效以目标格为中心缩小到 34%，420 毫秒后开始退场。点击格整块按音色着色，并显示大狗、哈基米或叮咚鸡的张嘴图片；长尾迹收起，保留小范围扩散环或 Emoji 弹出。
- 舞台顶部控制条使用单一“录制&分享”入口，独立面板分为“录制”“导入”和“曲库”：新录制默认 30 秒，可切换 60 秒；第一次发声建立录制零点，最后 5 秒状态条变红并逐秒倒计时。曲目名称使用 UTF-8，最多 8 字节；命名分享使用 `DGL1` 外层封装名称与原始 `DGT2` 数据，旧 `DGT1 / DGT2` 继续兼容。“生成分享链接”把命名分享码放进飞书妙搭 URL 的 `dj` 查询参数；接收页解析并校验后，在首次点击解锁音频后自动进入 DJ 模式播放。导入会自动恢复名称并存入 `localStorage` 曲库，曲库最多 50 首，可查看、改名、播放、导出分享码、生成飞书分享链接和删除。导入页可开启循环播放。按音频时钟保存双押、第三行长按、同 Deck 滑音与松开时间。`DGT2` 使用 4–7 字节头部、紧凑二进制、可选 LZSS、Base64url 与 CRC32，立体声只保存开关；最长 90 秒的旧录制仍可导入。回放通过 Web Audio 前瞻调度复现演奏，并用虚拟触点同步重现按下、长按、滑音、松手与跟手图片；结束后恢复原设置。
- 音游设置项显示为“音游模式（不好玩）”。
- 新增 `tools/verify_dj_recording.mjs`，覆盖 90 秒限制、编码往返、压缩、校验、双押、长按、滑音和播放事件顺序。
...
- 竖屏时 Deck 上下堆叠。
- DJ 模式固定使用普通 `4 × 3` 网格。
- 钢琴模式保留在单音色玩法中，并与 DJ 模式互斥；开启其中一个会直接切换演奏模式。
...
默认使用三台 Deck，三区顺序为狗、猫、鸡；切换到两台后保留左右两侧的大狗叫和叮咚鸡。
...
- 每个短音、长音纹理和释放尾音共用同一 Deck 立体声输出；开启时设置固定声像，关闭后回到中央。浏览器缺少 `StereoPannerNode` 时保持居中播放。
...
- Deck 数量写入云端后重建网格。
- 三个 Deck 音色分别写入对应云端键。
- DJ 模式开启时仍可点击钢琴按钮，并直接切换到钢琴模式。
- 钢琴模式开启时点击 DJ 按钮，会关闭钢琴模式并切换到 DJ。
...
- 自由 DJ 特效使用全屏画布，发散中心位于对应 Deck；音游使用同一画布的 34% 局部版本。当视觉需要严格限制在 Deck 内时，可给每台 Deck 增加裁剪区域或独立画布。
