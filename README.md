...
- 每局按当前 `128 BPM` 随机生成 8–24 组四小节乐句，共 `32–96` 小节，对应 1–3 分钟。
- 目标覆盖每台 Deck 的完整 `4 × 3` 网格；2 Deck 为 24 个区域，3 Deck 为 36 个区域，Deck 数量直接决定操作难度。
- 网格会在音游期间自动显示，八边形目标提前约 1.2 秒出现并收缩到目标格。
- 每组按“主题、重复、变奏、跨 Deck 对答、收束”组织；同一乐句的音高只停留或移动到相邻列。
- 音色口号会按固定音节顺序出现，包含“大狗叫”“大狗大狗叫叫叫”“叮咚叮咚鸡”，Deck 分配改变后跟随对应音色。
- 谱面包含单点、分布在不同 Deck 的双押，以及第三行延音区域的连音链。
- 连音链会分别判定起按、每次滑入相邻目标格和末端松手；提前松开、滑错区域或松晚都会中断。
- 连音期间只在其他 Deck 安排单点，整张谱面最多同时占用两根手指。
...
- 设置里的 `AUTO 自动演奏` 会按谱面自动短按、双押、拖动连音链并按时松手，沿用同一套声音、跟手特效与完整判定，可直接试听乐句生成效果。
