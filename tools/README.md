...
仍允许当前页面内切换的本地降级行为；DJ 跟手特效样式和立体声开关也会保存到独立云端键。

第六条命令会验证两台和三台 Deck 在横竖屏下都保留完整 `4 × 3` 网格、36 个
物理键映射、键盘按下与松开、按键重复抑制、设置面板输入屏蔽、页面失焦清理、
旋转时释放旧布局输入、每台 Deck 独立维持一个长音、网格开关与 PC 快捷键展示在 DJ 模式中
控制细网格，以及多指跟手尾迹的移动采样、长度上限、跨格脉冲、独立退场与
按 Deck 音色映射的大狗、猫、鸡图片样式。

第七条命令会验证录制面板默认 30 秒并提供 60 秒档位、三页曲库界面、分享码直接展示与复制、
飞书妙搭分享链接的生成与查询参数自动播放、导入循环播放、UTF-8 名称的 8 字节限制、
`DGL1` 命名分享和 `localStorage` 去重持久化，
以及格式层保留的 90 秒兼容限制。它也覆盖 DGT2 的 4–7 字节头部、
旧 DGT1 导入、紧凑二进制与 Base64url 往返、无损压缩和 CRC32 损坏检测，
并检查只保存立体声开关、双押、第三行长按、同 Deck 滑音、精确松开、两台 Deck
...
期间保留另一只手的单点空间。句内连奏会获得长于独立短音的自动发声时值。
测试也会检查纸屑、折线、几何雨、星星的随机缩小点击层，以及整格着色与对应角色图片、最多两指约束、起按与松手窗口、滑音节点、触控和键盘入口、自动演奏对
单点释放、真实拖动、双押和连音末端释放的调度。

第九条命令会验证顶部与设置菜单的立体声开关保持同步，一栏居中、两栏左右、
三栏左中右的固定声像，以及关闭后全部回中；同时确认音频路径只使用
`StereoPannerNode`，页面里已经移除原 3D 控制区和相关快捷键。
